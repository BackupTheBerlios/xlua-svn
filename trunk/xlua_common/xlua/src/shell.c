...
static void shell_help( char* args )
{
  ...
  printf( "  luac [args] - run the Lua compiler with given arguments\n" );
...
// 'luac' handler
static void shell_luac( char* args )
{
  int nargs = 0;
  char* luac_argv[ SHELL_MAX_LUA_ARGS + 2 ];
  char *p, *prev, *temp;
  ...
  if( *args )
  {
    ...
    while( p )
    {
      if( nargs == SHELL_MAX_LUA_ARGS )
      {
        printf( "Too many arguments to 'luac' (maxim %d)\n", SHELL_MAX_LUA_ARGS );
        return;
      }
      *p = 0;
      luac_argv[ nargs + 1 ] = temp = prev;
      nargs ++;
      prev = p + 1;
      p = strchr( p + 1, ' ' );
      // If the argument is quoted, remove the quotes and transform the 'alternate chars' back to space
  ...
  luac_argv[ nargs + 1 ] = NULL;
  luac_main( nargs + 1, luac_argv );
  clearerr( stdin );
}
      ...
      *p1 = 0;
      ...
      if( p2 )
      {
        *p2 = 0;
        ...
        if( ( fps = fopen( args, "r" ) ) == NULL )
          printf( "Unable to open %s for reading\n", args );
        else
        {
          if( ( fpd = fopen( p1 + 1, "w" ) ) == NULL )
